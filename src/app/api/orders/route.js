    ...
    try {
        ...
        if (clienteExistente.rows.length > 0) {
            ...
            const nuevoCliente = await client.query('INSERT INTO clientes (nombre, correo, telefono, usuario_id) VALUES ($1, $2, $3, $4) RETURNING id', [
                cliente_info.nombre,
                cliente_info.correo,
                cliente_info.telefono || '', // Usar el teléfono del cliente_info
                cliente_id,
            ]);
        ...
        const pricesRes = await client.query('SELECT id, nombre, precio, stock FROM productos WHERE id = ANY($1::int[])', [
            productIds
        ]);
        ...
        const productosConPrecioDB = productos.map((p) => {
            ...
            if (productFromDB.stock < p.cantidad) throw new Error(`Stock insuficiente para "${productFromDB.nombre}". Disponible: ${productFromDB.stock}, Solicitado: ${p.cantidad}.`);
        ...
        const pedidoRes = await client.query(
            'INSERT INTO pedidos (cliente_id, total, status) VALUES ($1, $2, $3) RETURNING id',
            [finalClientId, totalCalculado, 'Pendiente']
        );
        ...
        for (const producto of productosConPrecioDB) {
            ...
            // El trigger 'trg_actualizar_stock_venta' se encargará de actualizar el stock.
        }

        await client.query('COMMIT');
        return NextResponse.json({ message: 'Pedido creado exitosamente', pedidoId: pedidoId }, { status: 201 });
...
export async function DELETE(request) {
    ...
    try {
        ...
        // Opcional: Devolver el stock de los productos del pedido cancelado.
        // Esto requeriría una lógica más compleja para manejar devoluciones.
        // Por ahora, solo eliminamos el pedido y sus detalles.
